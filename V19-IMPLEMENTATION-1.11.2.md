...
冻干与激光条带为 `8 directions × 8 phases = 64 frames`。运行时通过 `directionIndex * 8` 选取对应连续八帧，不再使用四相硬编码。
...
桌面窗口提供 46px 高的可见对话感应条。点击后显示输入栏并把键盘焦点放入文本框。面板“大模型 → 对话联调”使用完全相同的：
...
| `normalFaded` / `unhappyFaded` | 暖棕褪色、旧银质感、局部轻锈迹 | 时间推进后进入 |
| `back` | 同圆心猫爪浮雕背面 | 双击翻转 |

拖拽优先级高于单击与双击。指针移动超过阈值后只移动窗口，不产生刷新或翻面动作。石化银币不受普通触摸手势影响。

## 9. 魔法与节日

V19 魔法四行：Accio Broom、Apparate、Petrificus Totalus、Scourgify。V19 节日四行：圣诞、万圣节、春节、主人生日。它们共用 V18/V19 身份比例，服饰仅作覆盖。

主人菜单明确触发的魔法使用 `OwnerForced`，可打断普通行为；笼中、旅行中和已石化等硬状态仍阻止不兼容魔法。自主魔法保留每日次数、冷却与资格评分。节日素材只在精确日期触发，预览不能绕过日期门禁。
...
Windows CI 还必须完成：Release 编译、测试运行器、WPF XAML 编译、自包含发布、安装器载荷校验和产物上传。1.11.2 安装器名称为 `Pupu-Setup-x64-1.11.2.exe`。
